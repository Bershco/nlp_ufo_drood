...
def make_visuals(freq: pd.DataFrame, by_chapter: pd.DataFrame, cooc: pd.DataFrame, sentiment: pd.DataFrame, suspects: pd.DataFrame) -> None:
    ...
    if not by_chapter.empty:
        ...
        fig.tight_layout()
        fig.savefig(FIGURES / "drood_mentions_by_chapter.png", dpi=300)
        plt.close(fig)
...
def run() -> None:
    ...
    ax.legend(fontsize=7, ncol=2); fig.tight_layout(); fig.savefig(FIGURES / "drood_dickens_motifs.png", dpi=300); plt.close(fig)
